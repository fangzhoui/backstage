...
export default {
	...
	methods: {
		...
		pick(id){
			...
			api_product.novelDetail({id: id}).then(res=> {
		...
		// 获取列表
		getType(obj){
			api_product.novelList(obj).then(res=> {
		...
		deleteItemList(id){
			this.$confirm('删除后将无法恢复，是否确定删除?').then(()=> {
				api_product.novelDelete({id: id}).then(res=> {
					if(res.data.code==0){
		...
		submitFn(){
			...
			this.$refs.novelFrom.validate(valid=> {
				if(valid){
					...
					if(!!_self.switchTurn){
						obj.id = _self.code;
						api_product.novelUpdate(obj).then(res=> {
							if(res.data.code == '0'){
						...
						api_product.novelInsert(obj).then(res=> {
							if(res.data.code == '0'){
								_self.$message({
						          message: '新增成功',
						          type: 'success'
